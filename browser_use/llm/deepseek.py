...
import logging
import traceback
from typing import Any, List, Dict, Optional, cast, Iterable, Union
...
MessageType = Union[ChatCompletionSystemMessageParam, ChatCompletionUserMessageParam, ChatCompletionAssistantMessageParam]
...
class DeepseekChat(BaseLLM):
    ...
    def _convert_message_to_dict(self, message: BaseMessage) -> MessageType:
    ...
    async def _agenerate(self, messages: List[BaseMessage], stop=None, run_manager=None, **kwargs):
        try:
            ...
            message_dicts: List[MessageType] = []
    ...
    def with_structured_output(self, output_schema, **kwargs):
        ...
        async def _process_messages(messages: List[BaseMessage]) -> dict:
            try:
                ...
                # Initialize message list with correct type
                message_dicts: List[MessageType] = []
                ...
                try:
                    ...
                    try:
                        ...
                        elif isinstance(parsed_dict['current_state'], dict):
                            state_dict = parsed_dict['current_state']
                            for field in ['evaluation_previous_goal', 'memory', 'next_goal']:
                                if field not in state_dict:
                                    state_dict[field] = ""
                            parsed_dict['current_state'] = AgentBrain(**state_dict).model_dump()
